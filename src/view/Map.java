  ...
  public Map() {
	...
	public void paintComponent(Graphics g){
		...
		for(Door door : doors) {
			...
			g.drawRect(door.getPosX()+doorWidth/4, door.getPosY()+doorHeight/3, door.getHitBox().getDeltaX()*2,door.getHitBox().getDeltaY()*2); //hitbox porte
		}
		for(GameObject obj : objects) {
			if(obj instanceof Opponent) {
				...
				g.drawRect(enemy.getPosX()+enemy.getHitBox().getDeltaX()*2, enemy.getPosY(), enemy.getHitBox().getDeltaX()*2,enemy.getHitBox().getDeltaY()*2); //hitbox du player
			...
			if(obj instanceof Player) {
				...
				g.setColor(Color.BLUE);
				g.drawRect(player.getPosX()+player.getHitBox().getDeltaX()*2, player.getPosY(), player.getHitBox().getDeltaX()*2,player.getHitBox().getDeltaY()*2); //hitbox du player
			}			
		}
    }
